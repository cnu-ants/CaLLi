...
struct
        ...
        (fun bb_name _ icfg ->
          ...
          | Some (Term.CallSite { callee; _ }) ->
              ...
              let icfg =
                ...
                | Some f_callee ->
                    let icfg' =
                      add bb_name (f_callee.entry :: M.find bb_name f.cfg) icfg
                    in
                    let next = List.nth (Cfg.next bb f.cfg) 0 in
                    if Bbpool.mem f_callee.exit !Bbpool.pool then
                      add
                        f_callee.exit
                        (next.bb_name :: M.find f_callee.exit icfg')
                        icfg'
                    else icfg'
                | None -> icfg
              in
  ...
  let preds (bb : Basicblock.t) icfg _ : Basicblock.t list =
    M.fold
      (fun from succs acc ->
        if List.exists (fun s -> String.equal s bb.bb_name) succs then
          Bbpool.find from !Bbpool.pool :: acc
        else acc)
      icfg []
  ...
  let term_to_string (t : Term.t) : string =
    ...
    | Term.Exit _ ->
        "exit"
    | Term.Switch { cond; default_succ; succ; _ } ->
        let cases =
          succ
          |> List.map (fun (e1, e2) ->
                 Printf.sprintf "%s %s" (expr_s e1) (expr_s e2))
          |> String.concat ", "
        in
        Printf.sprintf
          "switch %s %s [%s]"
  ...
  let bb_instrs (bb : Basicblock.t) : string list =
    let stmt_lines =
      bb.stmts |> List.map (fun (s : Stmt.t) -> inst_to_string s.inst)
    in
    match bb.term with
    | Some term -> stmt_lines @ [ term_to_string term ]
    | None -> stmt_lines

  let lookup_function (name : string) (m : Function.t Module.M.t) : Function.t =
    match Module.find_opt name m with
    | Some f -> f
    | None -> Module.find name m

  let function_bb_ids (func : Function.t) : string list =
    let ids =
      Cfg.fold
        (fun from succs acc ->
          let acc = SS.add from acc in
          List.fold_left
            (fun a to_ ->
              if Bbpool.mem to_ !Bbpool.pool then
                let bb = Bbpool.find_bb to_ in
                if String.equal bb.func_name func.function_name then SS.add to_ a
                else a
              else a)
            acc succs)
        func.cfg SS.empty
    in
    let ids =
      if Bbpool.mem func.entry !Bbpool.pool then SS.add func.entry ids else ids
    in
    let ids =
      if Bbpool.mem func.exit !Bbpool.pool then SS.add func.exit ids else ids
    in
    SS.elements ids

  let bb_json (id : string) : Yojson.Safe.t =
  ...
  let to_functions_json (m : Function.t Module.M.t) : Yojson.Safe.t =
    let items =
      Module.fold
        (fun _ (func : Function.t) acc ->
          let name = func.function_name in
          let block_count = List.length (function_bb_ids func) in
          ( name,
            `Assoc
              [
                ("name", `String name);
                ("block_count", `Int block_count);
                ("entry", `String func.entry);
                ("exit", `String func.exit);
              ] )
          :: acc)
        m []
  ...
  let to_block_index_json (m : Function.t Module.M.t) : Yojson.Safe.t =
    let items =
      Module.fold
        (fun _ (func : Function.t) acc ->
          let bbs = function_bb_ids func in
          List.fold_left
            (fun acc2 bb ->
              (`Assoc
                 [
                   ("id", `String bb);
                   ("func", `String func.function_name);
                 ])
              :: acc2)
            acc bbs)
        m []
  ...
  let to_cfg_json (m : Function.t Module.M.t) (func_name : string) : Yojson.Safe.t =
    let func = lookup_function func_name m in
    let node_ids = function_bb_ids func in
    ...
    let edges_json, _ =
      Cfg.fold
        (fun from succs (acc, idx) ->
          let from_bb = Bbpool.find_bb from in
          if String.equal from_bb.func_name func.function_name then
            List.fold_left
              (fun (acc2, idx2) to_ ->
                if Bbpool.mem to_ !Bbpool.pool then
                  let to_bb = Bbpool.find_bb to_ in
                  if String.equal to_bb.func_name func.function_name then
  ...
  let edge_kind_of
    ...
    | Some (Term.CallSite { callee; _ }) ->
        ...
        let fallbacks =
          Cfg.next from_bb f.cfg |> List.map (fun (b : Basicblock.t) -> b.bb_name)
        in
        if String.equal to_id (callee ^ "#entry") then "call"
        else if List.exists (fun x -> String.equal x to_id) fallbacks then
          "fallback"
        else "intra"
    | _ ->
        if String.length from_id >= 5
           && String.sub from_id (String.length from_id - 5) 5 = "#exit"
        then "ret"
        else "intra"
  ...
  let to_graph_json (m : Function.t Module.M.t) (icfg : t) : Yojson.Safe.t =
    let node_ids =
      M.fold
        (fun from succs acc ->
          let acc = SS.add from acc in
          List.fold_left (fun a s -> SS.add s a) acc succs)
        icfg SS.empty
    in
        ...
        (fun from succs (acc, idx) ->
          List.fold_left
            (fun (acc2, idx2) to_ ->
              ...
              ( edge_json ~id:eid ~source:from ~target:to_ ~kind :: acc2,
                idx2 + 1 ))
            (acc, idx) succs)
        icfg ([], 0)
    in
