...
let add_prune_node (f : Function.t) = 
  let cfg = 
    ...
    (fun bb_name _ cfg ->
      ...
      match bb.term with
      | Some CondBr {cond; succ0; succ1; _} ->
        ...
        | Name {name=cond_name;_}, Name {name=name0; _}, Name {name=name1; _} -> 
          ...
          let true_prune_bb : Basicblock.t = 
            {func_name=bb.func_name; bb_name=true_stmt.bb_name; stmts=[true_stmt]; term=Some(Br{bb_name=true_stmt.bb_name; succ=succ0}); loc=""} in
          let false_prune_bb : Basicblock.t = 
            {func_name=bb.func_name; bb_name=false_stmt.bb_name; stmts=[false_stmt]; term=Some(Br{bb_name=false_stmt.bb_name; succ=succ1}); loc=""} in
          let current_bb : Basicblock.t = 
            {bb with term=Some(CondBr {bb_name=bb_name; cond=cond; succ0=Name{ty=Label; name=true_prune_bb.bb_name}; succ1=Name{ty=Label; name=false_prune_bb.bb_name}})} in 
      ...
      | Some Switch {cond; succ; default_succ; _;} ->
        ...
        let prune_bb_list : Basicblock.t list = 
          ...
          (fun (bb' : Expr.t) (stmt : Stmt.t) -> 
            let bb' : Basicblock.t = {func_name=bb.func_name; bb_name=stmt.bb_name; stmts=[stmt]; term=Some(Br{bb_name=stmt.bb_name; succ=bb'}); loc=""} in
            let _ = Bbpool.pool := Bbpool.add stmt.bb_name bb' !Bbpool.pool in
            bb'
          )
          bb_list stmt_list in
        let default_bb : Basicblock.t = 
          {func_name=bb.func_name; bb_name=default_stmt.bb_name; stmts=[default_stmt]; term=Some(Br{bb_name=default_stmt.bb_name; succ=default_succ}); loc=""} in
        let _ = Bbpool.pool := Bbpool.add default_stmt.bb_name default_bb !Bbpool.pool in
        let current_bb = 
          {bb with term=Some(Switch {bb_name=bb_name; cond=cond; default_succ=Name{ty=Label; name=default_bb.bb_name};
