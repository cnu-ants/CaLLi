...
module type WEB_ANALYZER = sig
  ...
  val get_functions_json : unit -> Yojson.Safe.t
  val get_block_index_json : unit -> Yojson.Safe.t

  val get_callgraph_full_json : unit -> Yojson.Safe.t
  val get_callgraph_neighbors_json : string -> Yojson.Safe.t
  val get_callgraph_scc_json : unit -> Yojson.Safe.t

  val get_cfg_for_func_json : string -> Yojson.Safe.t
  ...
  val get_states_for_func_json : runtime -> string -> Yojson.Safe.t
  val get_callgraph_neighbors_meta_json : string -> Yojson.Safe.t
...
struct
  ...
  let env_json () : Yojson.Safe.t =
    ...
    Env.iter
      (fun k v ->
        items :=
          (`Assoc [ ("var", `String k); ("addr", `String v) ]) :: !items)
      !(Env.env);
  ...
  let add_meta
    ...
    match j with
    | `Assoc kv ->
        `Assoc (("ran", `Int ran) :: ("reason", `String reason) :: kv)
    | _ -> j
  ...
  let start
    ...
    let on_websocket (ws : Dream.websocket) : unit Lwt.t =
        ...
        if !paused then Lwt.return_unit
          ...
          | A.Stepped bb_name ->
              ...
              if SS.mem bb_name !bps then (
                stop ();
                send_json
                  (add_meta (A.snapshot_json runtime) ~ran:ran' ~reason:"breakpoint");
                Lwt.return_unit
              ) else if fuel <= 0 then
                Lwt.pause () >>= fun () -> run_continue max_steps_per_slice ran'
              else run_continue (fuel - 1) ran'
      ...
      and run_budget (n : int) (fuel : int) (ran : int) : unit Lwt.t =
        if !paused then Lwt.return_unit
          ...
          stop ();
          if ran > 0 then
            send_json (add_meta (A.snapshot_json runtime) ~ran ~reason:"step");
          Lwt.return_unit
        ) else
          match A.step_once runtime with
          | A.Done ->
          ...
          | A.Stepped _ ->
              ...
              else run_budget (n - 1) (fuel - 1) ran'
            ...
            send_json
              (`Assoc
                [
                  ...
                  ( "msg",
                    `String
                      ("Engine Error: " ^ Printexc.to_string exn) );
                ]);
            Lwt.return_unit)
      in
        ...
        Lwt.catch
          (fun () ->
            ...
            | Restart ->
                ...
                logf "restart end";
                send_json
                  (add_meta (A.snapshot_json runtime) ~ran:0 ~reason:"restart");
                Lwt.return_unit
            | BpSet { bb; enabled } ->
                bps :=
                  if enabled then SS.add bb !bps else SS.remove bb !bps;
                send_json (breakpoints_json !bps);
                Lwt.return_unit
            | BpSync { bbs } ->
                bps :=
                  List.fold_left (fun acc x -> SS.add x acc) SS.empty bbs;
                send_json (breakpoints_json !bps);
                Lwt.return_unit)
          (fun exn ->
      ...
      let rec cmd_processor_loop () : unit Lwt.t =
        if !closed && Queue.is_empty cmd_queue then (
          ...
          handle_cmd c >>= fun () -> cmd_processor_loop ()
      in
    ...
    let function_states_handler req =
      match Dream.query req "func" with
      | None -> Dream.respond ~status:`Bad_Request "missing query param: func"
      | Some fn ->
          Dream.json (Yojson.Safe.to_string (A.get_states_for_func_json runtime fn))
    in

    let functions_handler _req =
      Dream.json (Yojson.Safe.to_string (A.get_functions_json ()))
    in

    let block_index_handler _req =
      Dream.json (Yojson.Safe.to_string (A.get_block_index_json ()))
    in

    let callgraph_handler req =
      let mode = Option.value (Dream.query req "mode") ~default:"neighbors" in
      match mode with
      | "full" ->
          Dream.json (Yojson.Safe.to_string (A.get_callgraph_full_json ()))
      | "scc" ->
          Dream.json (Yojson.Safe.to_string (A.get_callgraph_scc_json ()))
      | "neighbors" -> (
          match Dream.query req "func" with
          | Some fn when String.trim fn <> "" ->
              Dream.json
                (Yojson.Safe.to_string (A.get_callgraph_neighbors_json fn))
          | _ ->
              Dream.json (Yojson.Safe.to_string (A.get_callgraph_full_json ())) )
      | _ ->
          Dream.respond ~status:`Bad_Request "invalid mode: use full|neighbors|scc"
    ...
    let cfg_handler req =
      match Dream.query req "func" with
      | None ->
          Dream.respond ~status:`Bad_Request "missing query param: func"
      | Some fn ->
          Dream.json (Yojson.Safe.to_string (A.get_cfg_for_func_json fn))
    in
    let callgraph_meta_handler req =
  let mode = Option.value (Dream.query req "mode") ~default:"neighbors" in
  match mode with
  | "neighbors" -> (
      match Dream.query req "func" with
      | None -> Dream.respond ~status:`Bad_Request "missing query param: func"
      | Some fn ->
          Dream.json (Yojson.Safe.to_string (A.get_callgraph_neighbors_meta_json fn)) )
  | _ ->
      Dream.respond ~status:`Bad_Request "callgraph_meta currently supports only mode=neighbors"
    in
    ...
    @@ Dream.router
         ([
            Dream.get "/functions" functions_handler;
            Dream.get "/block_index" block_index_handler;
            Dream.get "/callgraph" callgraph_handler;
            Dream.get "/cfg" cfg_handler;

            Dream.get "/state" state_handler;
            Dream.get "/states" states_handler;
            Dream.get "/function_states" function_states_handler;
            Dream.get "/env" (fun _req ->
                Dream.json (Yojson.Safe.to_string (env_json ())));
            Dream.get "/ws" (fun _req -> Dream.websocket on_websocket);
            Dream.get "/callgraph_meta" callgraph_meta_handler;  
        ]
         @ extra_routes)
end
